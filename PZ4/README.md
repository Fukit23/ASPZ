...
### Завдання 4.5: 
Змодельовано ситуацію, коли realloc не може розширити блок пам'яті (було надіслано запит на максимальний розмір SIZE_MAX, прихований від компілятора модифікатором volatile, щоб уникнути оптимізації). Доведено, що старий блок при помилці realloc не звільняється системою. Для уникнення витоку пам'яті (Memory Leak) реалізовано ручне звільнення старого вказівника через free().
